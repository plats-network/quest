...
(function ($) {
    ...
    var attributeDefaults = {
        ...
        input: undefined,
        // the jQuery selector of the label field under the context of the form
        label: undefined,
    ...
    var findLabel = function ($form, attribute) {
        var $label = $form.find(attribute.label);
        if ($label.length && $label[0].tagName.toLowerCase() === 'div') {
            // checkbox list or radio list
            return $label.find('label');
        } else {
            return $label;
        }
    };

    //is-valid class is added to the input element when the validation passes
    //is-invalid class is added to the input element when the validation fails
    var updateAriaInvalid = function ($form, attribute, hasError) {
        if (attribute.updateAriaInvalid) {
            $form.find(attribute.input).addClass(hasError ? 'bg-red-50 border border-red-500 text-red-900' : 'bg-green-50 border border-green-500 text-green-900');
            $form.find(attribute.input).removeClass(hasError ? 'bg-green-50 border border-green-500 text-green-900' : 'bg-red-50 border border-red-500 text-red-900');
            $form.find(attribute.input).attr('aria-invalid', hasError ? 'true' : 'false');
            //label
            $form.find(attribute.label).addClass(hasError ? 'text-red-700' : 'text-green-700');
            $form.find(attribute.label).removeClass(hasError ? 'text-green-700' : 'text-red-700');
        }
    }

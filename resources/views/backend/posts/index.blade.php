...
@section('content')
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-4">
        <div class="d-block mb-4 mb-md-0">
            ...
            <h2 class="h4">All {{__('posts.title')}}</h2>
        ...
        <div class="row mt-4">
            <div class="col">
                <table id="datatable" class="table table-bordered table-hover table-responsive-sm">
                    ...
                    <tbody>
                        @foreach($$module_name as $itemPost)
                        <tr>
                            <td>
                                {{ $itemPost->id }}
                            </td>
                            <td>
                                <strong>{{ $itemPost->name }}</strong>
                                <br>
                                <small class="text-muted">Updated At: {{ $itemPost->updated_at->diffForHumans() }}</small>
                            </td>
                            <td>
                                {{ $itemPost->category_name }}
                            </td>
                            <td>
                                {{ $itemPost->type }}
                            </td>
                            <td>
                                <img src="{{ asset($itemPost->featured_image) }}" class="img-fluid img-thumbnail" style="max-width:200px;" alt="{{ $itemPost->name }}">
                            </td>
                            <td class="text-end">
                                <a href='{!!route("backend.posts.edit", $itemPost)!!}' class='btn btn-sm btn-primary mt-1' data-toggle="tooltip" title="Edit {{ ucwords(Str::singular($module_name)) }}"><i class="fas fa-wrench"></i></a>
                                <a href='{!!route("backend.posts.create", $itemPost)!!}' class='btn btn-sm btn-primary mt-1' data-toggle="tooltip" title="Edit {{ ucwords(Str::singular($module_name)) }}"><i class="fas fa-wrench"></i></a>
                                <a href='{!!route("backend.posts.show", $itemPost)!!}' class='btn btn-sm btn-success mt-1' data-toggle="tooltip" title="Show {{ ucwords(Str::singular($module_name)) }}"><i class="fas fa-tv"></i></a>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>

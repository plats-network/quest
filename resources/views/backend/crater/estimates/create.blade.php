    ...
    <div class="py-4">
        <nav aria-label="breadcrumb" class="d-none d-md-inline-block">
            <ol class="breadcrumb breadcrumb-dark breadcrumb-transparent">
                ...
                <li class="breadcrumb-item" ><a href="{{route('admin.estimates.index')}}">{{__('crater.estimates.title')}}</a></li>
        ...
        <div class="d-flex justify-content-between w-100 flex-wrap">
            ...
            <div>
                <a href="{{route('admin.estimates.index', ['key_param' => 1])}}"
                        ...
                        <div class="col-sm-12">
                            <div>
                                ...
                                <form method="post" id="post_form"
                                      action="{{ $type == 1 ? route('admin.estimates.store'): route('admin.estimates.update', $estimate) }}">
                                    @if($type ==2)
                                        @method('PATCH')
                                    @endif
                                    @csrf
                                        ...
                                        <div class="row mb-3">
                                            <div class="col-md-6">
                                                <div class="form-group field-name">
                                                    <label for="city">{{__('crater.estimates.customer')}}</label>
                                                    <select class="form-select w-100 mb-0" id="customerInvoice" name="customer_id"
                                                        ...
                                                        @foreach($customers as $key=> $customerItem)
                                                            <option
                                                                value="{{$customerItem->id}}" {!! (($key == Request::get('customer_id') || ($customer->id == $customerItem->id))) ? ' selected="selected"' : '' !!} >{{$customerItem->name}}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            </div>
                                        ...
                                        <h2 class="h5 my-4">{{__('crater.estimates.title')}}</h2>

                                        <div class="row">
                                            <div class="col-md-6">
                                                <div class="form-group mb-3">
                                                    <label class="form-label"
                                                           for="first_name"><b>{{__('crater.estimates.status')}}
                                                            :</b></label>
                                                    <input type="text" class="form-control @error('evaluate_quality') is-invalid @enderror" name="estimate_status"
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="form-group mb-3">
                                                <label class="form-label"
                                                       for="first_name"><b>{{__('crater.estimates.date')}}
                                                        :</b></label>
                                                <input type="text" class="form-control @error('evaluate_quality') is-invalid @enderror" name="estimate_date"
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group mb-3">
                                                <label class="form-label"
                                                       for="first_name"><b>{{__('crater.estimates.due_date')}}
                                                        :</b></label>
                                                <input type="text" class="form-control @error('evaluate_quality') is-invalid @enderror" name="due_date"
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="form-group mb-3">
                                                <label class="form-label"
                                                       for="first_name"><b>{{__('crater.estimates.number')}}
                                                        :</b></label>
                                                <input type="text" class="form-control @error('evaluate_quality') is-invalid @enderror" name="estimate_number"
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group mb-3">
                                                <label class="form-label"
                                                       for="first_name"><b>{{__('crater.estimates.ref_number')}}
                                                        :</b></label>
                                    ...
                                    <table class="table">
                                        ...
                                        <tr>
                                            ...
                                            <th scope="col" class="border-bottom">{{__('crater.estimates.item.title')}}</th>
                                            <th scope="col" class="border-bottom">{{__('crater.estimates.item.quantity')}}</th>
                                            <th scope="col" class="border-bottom">{{__('crater.estimates.item.price')}}</th>
                                            <th scope="col" class="border-bottom">{{__('crater.estimates.item.amount')}}</th>
                                            <th scope="col" class="border-bottom">{{__('crater.general.actions')}}</th>
                                        </tr>
                                        </thead>
                                        <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td style="width: 12%">
                                                <input type="number" min="1" max="100" value="{{$item->quantity}}" class="form-control itemQuantity" id="inputQuantity{{$item->id}}">
                                            </td>
                                            <td style="width: 12%">
                                                <input type="text" value=" {{format_money_vnd($item->price)}}" class="form-control itemQuantity" id="inputQuantity{{$item->id}}">
                                            </td>
                                            <td>{{format_money_vnd($item->total)}}</td>
                                            <td>
                                                    <button class="btn btn-danger btn-sm" type="button"
                                                            data-item_id="{{$item->id}}"
                                                            data-estimate_id="{{$estimate->id}}">{{__('crater.general.delete')}}</button>
                                            </td>
                                        </tr>
                                        @endforeach
                                        </tbody>
                                    ...
                                    <div class="row">
                                            ...
                                            <div class="card">
                                                <div class="card-body">
                                                    <div class="d-flex justify-content-between bd-highlight mb-3">
                                                        <div
                                                            class="p-2 bd-highlight textSummary">{{__('crater.estimates.sub_total')}}</div>
                                                        <div class="p-2 bd-highlight"></div>
                                                        <div class="p-2 bd-highlight ">{{format_money_vnd($estimate->sub_total)}}</div>
                                                    </div>
                                                    <div class="d-flex justify-content-between bd-highlight mb-3">
                                                        <div
                                                            class="p-2 bd-highlight textSummary">{{__('crater.estimates.discount')}}</div>
                                                        <div class="p-2 bd-highlight"></div>
                                                        <div class="p-2 bd-highlight ">
                                                    ...
                                                    <div class="d-flex justify-content-between bd-highlight mb-3">
                                                        <div
                                                            class="p-2 bd-highlight textSummary">{{__('crater.estimates.total')}}</div>
                                                        <div class="p-2 bd-highlight"></div>
                                                        <div class="p-2 bd-highlight ">{{format_money_vnd($estimate->total)}}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        <div class="row">
                                            <div class="col-md-4">
                                                <div class="mb-3">
                                                    <label for="disabledSelect" class="form-label">{{__('crater.estimates.template')}}</label>
                ...
                <div class="modal-body p-0">
                    <div class="card p-3 p-lg-4">
                        ...
                        <div class="text-center text-md-center mb-4 mt-md-0">
                            <h1 class="mb-0 h4">{{__('crater.estimates.add_item')}}</h1>
                        </div>
                        <form method="POST" action="{{route('admin.postInvoiceAddItem', $estimate)}}" class="mt-4">
                        @csrf
                            <!-- Form -->
                            <div class="form-group mb-4">
                                <label for="email">{{__('crater.estimates.item.title')}}</label>
                            ...
                            <div class="d-grid">
                                <button type="submit" class="btn btn-primary">{{__('crater.general.add_new_item')}}</button>
                            </div>
                        </form>
                    </div>
                </div>
        ...
        jQuery(document).ready(function ($) {
            ...
            * */
            ...
            $('.btnAddItem').on('click', function () {
                ...
                let estimate_id = {{$estimate->id? $estimate->id: 1}};
                let week_key =2;
                let type = 4
                let value = 2;

                dataSend = {id: row_id, invoice:estimate_id, type: type, value: value, _token: _token};
